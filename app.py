...
def main():
    ...
    for i, t_label in enumerate(tabs):
        ...
        if t_cols[i].button(t_label, key=f"tab_spot_{i}", use_container_width=True, 
    ...
    if st.session_state.active_tab == "📊 지역 분석":
        sel_date = st.selectbox("📅 기준 날짜 선택", date_list, index=len(date_list)-1, key=f"spot_d1_{st.session_state.date_reset_key}")
        sel_regions = st.multiselect("🔍 비교 지역 선택", region_list, default=[region_list[0]] if region_list else [])
        
        if sel_regions:
            curr_idx = date_list.index(sel_date)
            # 스포트라이트 대상 추출 (8주 누적 상승 TOP 10)
            ...
            st.plotly_chart(px.line(df_maemae.iloc[max(0, curr_idx-7):curr_idx+1][['날짜']+sel_regions], x='날짜', y=sel_regions, markers=True, color_discrete_sequence=c_palette).update_layout(height=320, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)'), use_container_width=True, config={'staticPlot': True})
            st.markdown('<div class="chart-title">📉 전세 증감 추이 (최근 8주)</div>', unsafe_allow_html=True)
            st.plotly_chart(px.line(df_jeonse.iloc[max(0, curr_idx-7):curr_idx+1][['날짜']+sel_regions], x='날짜', y=sel_regions, markers=True, color_discrete_sequence=c_palette).update_layout(height=320, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)'), use_container_width=True, config={'staticPlot': True})
        ...
        sel_date_rank = st.selectbox("📅 랭킹 기준일 선택", date_list, index=len(date_list)-1, key=f"spot_d3_{st.session_state.date_reset_key}")
    ...
    if st.button("🚪 **안전하게 앱 종료하기**", key="exit_v_final_spot", use_container_width=True):
