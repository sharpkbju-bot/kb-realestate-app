...
def main():
    ...
    # 4. 투자 심층 분석 탭
        ...
        if analysis_type == "전세가율(Gap) 흐름 추적기":
            ...
            st.markdown('<p style="color:#333; font-size:14px; text-align:center;">매매 증감의 <b>4주(단기) 및 12주(장기) 이동평균선</b>을 통해 시장의 방향성을 진단합니다.</p>', unsafe_allow_html=True)
            sel_region_ma = st.selectbox("📌 분석 지역 선택", region_list, key=f"ma_reg_{st.session_state.date_reset_key}")
            
            cum_m_ma = df_maemae[sel_region_ma].cumsum()
            ma4 = cum_m_ma.rolling(window=4).mean()
            ...
            if len(ma4) >= 2 and pd.notna(ma4.iloc[-1]) and pd.notna(ma12.iloc[-1]):
                if ma4.iloc[-2] <= ma12.iloc[-2] and ma4.iloc[-1] > ma12.iloc[-1]:
                    cross_status = "🔥 골든크로스 발생 (상승 전환 기대)"
                elif ma4.iloc[-2] >= ma12.iloc[-2] and ma4.iloc[-1] < ma12.iloc[-1]:
                    cross_status = "❄️ 데드크로스 발생 (조정 진입 우려)"
                elif ma4.iloc[-1] > ma12.iloc[-1]:
                    cross_status = "📈 단기 상승 추세 유지 (MA4 > MA12)"
                else:
                    cross_status = "📉 단기 하락 추세 유지 (MA4 < MA12)"
            ...
            if "광명시" in region_list:
                ...
                cum_m_gm = df_maemae["광명시"].sum()
                cum_j_gm = df_jeonse["광명시"].sum()
                
                c_gm1, c_gm2 = st.columns(2)
                with c_gm1:
                    st.markdown(f'<div class="stat-card m-card"><div>광명시 전체 누적 매매</div><div class="stat-value">{cum_m_gm:+.2f}%</div></div>', unsafe_allow_html=True)
                with c_gm2:
                    st.markdown(f'<div class="stat-card j-card"><div>광명시 전체 누적 전세</div><div class="stat-value">{cum_j_gm:+.2f}%</div></div>', unsafe_allow_html=True)
