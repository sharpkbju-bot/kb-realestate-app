...
st.markdown("""
    ...
    * { font-family: 'Noto Sans KR', sans-serif !important; }
    ...
    /* 드롭다운 (라벨: 20px -> 17px, 선택값: 22px -> 18px) */
    ...
    /* 통계 카드 (메인수치: 28px -> 24px, 텍스트: 18px -> 15px) */
...
def main():
    ...
    if st.session_state.active_tab == "📊 지역 분석":
        sel_date = st.selectbox("📅 기준 날짜 선택", date_list, index=len(date_list)-1, key=f"ds1_{st.session_state.date_reset_key}")
