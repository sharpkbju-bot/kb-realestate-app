...
st.markdown("""
    ...
    .title-container { padding: 40px 0 25px 0; text-align: center; width: 100%; }
...
def create_chart(df, region, date, color):
    ...
    # 에러 수정: marker 속성 구조 변경
    fig.update_traces(line_color=color, line_width=4, marker=dict(size=12, line=dict(width=2, color='white')))
    fig.update_layout(height=240, margin=dict(l=15, r=15, t=10, b=10), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', xaxis=dict(showgrid=False, title=""), yaxis=dict(showgrid=True, gridcolor='#f2f2f2', title=""), hovermode="x unified")
    return fig
