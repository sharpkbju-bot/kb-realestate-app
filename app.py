...
# UI 디자인 통합 스타일시트 (디자인 복구 핵심)
st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@300;500;700;900&display=swap');
    
    /* 1. 전체 텍스트 900 BOLD 강제 적용 */
    * { font-family: 'Noto Sans KR', sans-serif !important; }
    ...
    /* 3. 탭 메뉴 디자인 복구 (BOLD 900 + 글자 크기) */
    .stButton > button { 
        width: 100% !important; height: 65px !important; border-radius: 0px !important; 
        border: 2px solid #2c3e50 !important; font-weight: 900 !important; font-size: 19px !important;
    ...
    .stat-card { padding: 15px; border-radius: 12px; margin: 10px 0; display: flex; flex-direction: column; align-items: center; background: white; box-shadow: 0 4px 6px rgba(0,0,0,0.1); border: 1.5px solid #cccccc; }
    .m-card { border-left: 12px solid #FF4500; color: #D32F2F; }
    .j-card { border-left: 12px solid #01579B; color: #01579B; }

    /* 6. 차트 타이틀 */
    .chart-title { font-size: 20px; font-weight: 900; color: #ffffff !important; background: #2c3e50; border-radius: 8px; text-align: center; padding: 12px; margin: 25px 0; border: 2px solid #FFD700; }
    
    /* 7. 랭킹 카드 컬러 복구 (핵심 요청 사항) */
...
def main():
    ...
    for i, t_label in enumerate(tabs):
        ...
        if t_cols[i].button(t_label, key=f"tab_fix_{i}", use_container_width=True, 
    ...
    if st.session_state.active_tab == "📊 지역 분석":
        sel_date = st.selectbox("📅 기준 날짜 선택", date_list, index=len(date_list)-1, key=f"tab1_date_{st.session_state.date_reset_key}")
    ...
    # 3. 누적 랭킹 (컬러 및 굵기 완벽 복구)
    elif st.session_state.active_tab == "🏆 누적 랭킹 TOP 10":
        sel_date_rank = st.selectbox("📅 랭킹 기준일 선택", date_list, index=len(date_list)-1, key=f"tab3_date_{st.session_state.date_reset_key}")
