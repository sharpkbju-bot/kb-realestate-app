...
def main():
    ...
    if st.session_state.active_tab == "📊 지역 분석":
        ...
        if sel_regions:
            ...
            # ★ AI 분석 테이블 (전세 그래프 아래쪽으로 위치 변경) ★
            st.markdown(f'<div class="chart-title" style="background:#4B0082; border-color:#E6E6FA; margin-top:30px;">🧐 {start_date[5:]} ~ {end_date[5:]} 기간 심층 분석</div>', unsafe_allow_html=True)
