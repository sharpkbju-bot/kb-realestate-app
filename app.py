...
st.markdown("""
    ...
    .highlight-card {
        ...
        border: 3px solid #FF4500 !important; /* 오렌지색 두꺼운 테두리 */
    }
        ...
        if sel_regions:
            ...
            for region in sel_regions:
                ...
                with col1:
                    ...
                    st.markdown(f'<div class="{m_class}"><div>{region} 매매</div><div style="font-size:24px;">{m_val:+.2f}%</div></div>', unsafe_allow_html=True)
                with col2:
                    j_class = "stat-card j-card highlight-card" if is_j_hot else "stat-card j-card"
                    st.markdown(f'<div class="{j_class}"><div>{region} 전세</div><div style="font-size:24px;">{j_val:+.2f}%</div><div style="color:#FF4500; font-size:12px;">🔥 누적TOP</div></div>', unsafe_allow_html=True)
